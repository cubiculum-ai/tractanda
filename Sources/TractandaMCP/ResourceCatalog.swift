...
enum ResourceCatalog {
    ...
    static let references: [Reference] = [
        ...
        .init(
            ...
            text: """
                Profile: tractanda.spotlight.v0. Examples: classID == "Item"; subject ==[cd] "*chess*"; (priority == "P1" || priority == "P2"). Item is the concrete generic root. classID == "Item" selects generic items only; kMDItemContentTypeTree == "Item" includes every subclass. Queries already exclude deleted items. Default ordering is most recently modified first, with ascending ItemID only for equal timestamps. An explicit saved-view sort overrides this default.
