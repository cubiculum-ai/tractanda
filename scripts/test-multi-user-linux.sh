...
# Root is confined to this disposable container. CAP_SYS_ADMIN is used only for the archive bind mount.
container run --rm --name "tractanda-peers-$(basename "$run_dir")" --cpus 4 --memory 4G \
  ...
  --workdir /workspace "$image_ref" python3 scripts/verify-multi-user.py /build/debug/tractanda \
  --provision-disposable-linux --mcp-binary /build/debug/tractanda-mcp --output /results/multi-user.json
printf 'Multi-user Linux evidence: %s\n' "$run_dir"
