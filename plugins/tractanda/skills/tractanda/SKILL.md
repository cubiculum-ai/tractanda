...
Saved views are ordinary items carrying `viewDefinition`; use a supported class such as `Item` for a new one. Their ordered `viewDefinition.presentation.sections` references define section order. An implicit project board uses that as a preferred order, then appends other status leaves; without it, sibling `categoryOrder` values and names determine traversal order. Set integer `categoryOrder` values deliberately when extending an ordered status taxonomy. None of the status names is built into the client. Membership has no intrinsic item order. Kanban preserves native query order. Priority and urgency are category memberships, ordered through their root's immediate children by `categoryOrder`. The sample Project view supplies the default priority sort; a project's own sort overrides it. Query sort entries choose `property` or `categoryRootID` (plain UUID), with `isAscending`; stored view sorts/columns use a tagged current reference for `categoryRootID`. Unranked items sort last. Use `tractanda_memberships` for effective displayed branches; raw overrides alone omit rule/inherited/personal membership. `sortOrder` is optional explicit view metadata, never generated by browser saves.
